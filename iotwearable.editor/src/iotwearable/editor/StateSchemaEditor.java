...
import iotwearable.validation.NameValidator;
...
public class StateSchemaEditor extends GraphicalEditorWithPalette {
	...
	protected void configureGraphicalViewer() {
		...
		getGraphicalViewer().setContextMenu(new StateSchemaContextMenu(this,getGraphicalViewer(), getActionRegistry()));
		// D&D
	...
	public class UnwrappingPropertySource implements IPropertySource {
		...
		public void setPropertyValue(Object id, Object value) {
			if(id.equals("name"))
			{
				NameValidator nameValidator = new NameValidator(value.toString());
				if(!nameValidator.isValid())
				{
					MessageWindow.show("Change Name ", nameValidator.getInvalidMessage());
					return;
				}
				if(!checkDuplicateName(value.toString()))
				{
					MessageWindow.show("Change Name ", value.toString() + " is existed" );
					return;
				}
			}
	...
	private boolean checkDuplicateName(String value) {
		for(StateComponent component : schema.getComponents())
		{
			if(component.getName()!=null)
			{
				if(!component.getName().isEmpty() && component.getName().equals(value))
					return false;
			}
		}
		return true;
